...
SpaghettiCaveCarver::SpaghettiCaveCarver() { SetConfig(SpaghettiCaveConfig{}); }
...
bool SpaghettiCaveCarver::ShouldCarve(int worldX, int worldY, int worldZ,
  ...
  // Don't carve near surface (protect top layers from holes) - Minecraft uses
  // ~4 blocks
  ...
  if (worldY > Config::CAVE_FADE_START_Y) {
    ...
    float fadeProgress =
        static_cast<float>(worldY - Config::CAVE_FADE_START_Y) /
